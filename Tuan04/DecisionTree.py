...
class Node:
    def __init__(self, feature=None, threshold=None, left=None, right=None, *, value=None): # những tham số sau '*' cần khai báo rõ tên đối số
        self.feature = feature  # feature dùng để chia node
        self.threshold = threshold  # ngưỡng chia node
...
class DecisionTreeClass:
    ...
    def predict(self, X):  # hàm dự đoán cho tất cả mẫu trong X
        return np.array([self.traverse_tree(x, self.root) for index, x in X.iterrows()]) # x là dạng Series
